#!/usr/bin/env python3
"""Test 10s early attach stealth bypass with periodic screenshots."""
import frida
import time
import subprocess
import tempfile
...
def main():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix="_maps.txt", delete=False) as tmp:
        tmp_path = tmp.name
    stdout, _, rc = adb_shell(f"su -c 'cat /proc/{pid}/maps'")
    if rc == 0:
        lines = stdout.splitlines()
        filtered = [line for line in lines if "frida" not in line.lower() and "gum" not in line.lower()]
        with open(tmp_path, "w") as f:
            f.write("\n".join(filtered))
        subprocess.run([ADB, "-s", DEVICE_SERIAL, "push", tmp_path, "/data/local/tmp/fake_maps_clean"], capture_output=True)
    os.unlink(tmp_path)

    with open("frida_bypass_stealth.js", "r") as f:
        script_src = f.read()
    script_src = script_src.replace('"/data/local/tmp/fake_maps"', '"/data/local/tmp/fake_maps_clean"')

    print(f"Attaching stealth bypass to PID {pid} at T+10s...")
    try:
    ...
    print("Capturing screenshots every 10s for 90s...")
    for i in range(9):
        time.sleep(10)
