// Emulator bypass for PUBG Mobile VNG on LD Player
// Combines local property/file hooks + network ANOGS patch + maps filtering
...
function safeReadUtf8(ptr) {
    try { return ptr.readUtf8String(); } catch(e) { return null; }
}
...
function hookPropertyGet() {
    try {
        ...
        Interceptor.attach(propGet.address, {
            onEnter: function(args) { this.key = safeReadUtf8(args[0]); this.buf = args[1]; },
...
// Hide Frida from maps reads by hooking strstr/strcasestr
function hookStrStr() {
    try {
        var libc = Process.findModuleByName("libc.so");
        if (!libc) return;
        var strstr = libc.findExportByName("strstr");
        if (strstr && !strstr.isNull()) {
            var origStrstr = new NativeFunction(strstr, 'pointer', ['pointer', 'pointer']);
            Interceptor.replace(strstr, new NativeCallback(function(haystack, needle) {
                var n = safeReadUtf8(needle);
                if (n && (n.indexOf("frida") !== -1 || n.indexOf("gadget") !== -1)) {
                    return ptr(0);
                }
                return origStrstr(haystack, needle);
            }, 'pointer', ['pointer', 'pointer']));
            log("Hooked strstr (anti-frida)");
        }
    } catch(e) { log("hookStrStr error: " + e); }
}
